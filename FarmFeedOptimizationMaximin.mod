...
# Maksyminowa skalarna funkcja celu
param w1 := 0.5;
param w2 := 0.5;

var min_value;

maximize MaximinObjective:
    min_value;

s.t. MaximinConstraint1:
    min_value <= w1 * (energy1 * x1 + energy2 * x2);

s.t. MaximinConstraint2:
    min_value <= w2 * (max_budget - (cost1 * x1 + cost2 * x2));
